(function () {
  ...
  (window["webpackJsonp"] = window["webpackJsonp"] || []).push([["pages-chat-chat-module"], {
      ...
      var ChatPage = /*#__PURE__*/function () {
        ...
        _createClass(ChatPage, [{
          ...
          value: function uploadImage(event) {
            ...
            this.chatService.imgName = file.name; // Storage path

            var fileStoragePath = "filesStorage/".concat(new Date().getTime(), "_").concat(file.name); // Image reference

            var imageRef = this.chatService.afStorage.ref(fileStoragePath); // File upload task

            this.chatService.fileUploadTask = this.chatService.afStorage.upload(fileStoragePath, file); // Show uploading progress

            this.chatService.percentageVal = this.chatService.fileUploadTask.percentageChanges();
            this.chatService.trackSnapshot = this.chatService.fileUploadTask.snapshotChanges().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_5__["finalize"])(function () {
              // Retreive uploaded image storage path
              _this3.chatService.UploadedImageURL = imageRef.getDownloadURL();

              _this3.chatService.UploadedImageURL.subscribe(function (resp) {
                _this3.storeFilesFirebase({
                  name: file.name,
                  filepath: resp,
                  size: _this3.chatService.imgSize
                });

                _this3.chatService.isFileUploading = false;
                _this3.chatService.isFileUploaded = true;
              }, function (error) {
                console.log(error);
              });
            }), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_5__["tap"])(function (snap) {
              _this3.chatService.imgSize = snap.totalBytes;
            }));
          }
        }, {
          key: "storeFilesFirebase",
          value: function storeFilesFirebase(image) {
            var fileId = this.chatService.afs.createId();
            this.chatService.filesCollection.doc(fileId).set(image).then(function (res) {
              console.log(res);
            })["catch"](function (err) {
              console.log(err);
            });
    ...
    /***/
    function srcAppPagesChatFormatFileSizePipeTs(module, __webpack_exports__, __webpack_require__) {
      "use strict";
